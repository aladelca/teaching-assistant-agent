...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Batch grading CLI for student folders (apellidos_nombres)"
    )
    ap.add_argument(
        "--submissions-root", required=True, help="Root path with one folder per student"
    )
    ap.add_argument(
        "--student-key-regex", default=r"^[^/]+_[^/]+$", help="Regex for valid folder names"
    )
    ap.add_argument(
        "--notebook-glob",
        default="**/*.ipynb",
        help="Notebook discovery glob inside student folder",
    )
    ...
    ap.add_argument(
        "--summary-csv", default="gradebook_summary.csv", help="Nombre de CSV consolidado"
    )
    ap.add_argument(
        "--gradebook-column", default="Final Score", help="Nombre de columna para nota final"
    )
    ...
    ap.add_argument(
        "--prompt", default="mvp_agent/prompts/evaluator_prompt.txt", help="Prompt de evaluación"
    )
    ...
    ap.add_argument(
        "--exec-mode", choices=["local", "docker"], default="local", help="Modo de ejecución"
    )
